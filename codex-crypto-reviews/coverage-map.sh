...
#
# Inputs:
#   codex-crypto-reviews/upstream-snapshots/*/ts-source/__tests__/*-test*.ts
#                                 — upstream test files (extracted npm tarball,
#                                   not the test() blocks inside compiled output)
...
UPSTREAM_DIR="$REVIEWS_DIR/upstream-snapshots"
TESTS_DIR="$REPO_ROOT/tests"
NA_FILE="$REVIEWS_DIR/coverage-map.na.tsv"
TSV_OUT="$REVIEWS_DIR/coverage-map.tsv"
MD_OUT="$REVIEWS_DIR/coverage-map.md"
...

{
  ...
  echo ""
  echo "Generated by [\`coverage-map.sh\`](./coverage-map.sh)."
  echo ""
  echo "Statuses:"
  echo ""
  echo "- **REVIEWED** — at least one C# \`[Fact]/[Theory]\` method has a"
  echo "  matching \`/// upstream: <relpath>:<line>\` annotation."
  echo "- **N/A** — the upstream test is intentionally not ported; the reason"
  echo "  is recorded in [\`coverage-map.na.tsv\`](./coverage-map.na.tsv)."
  echo "- **MISSING** — neither of the above. PR-3 must either add C# coverage"
