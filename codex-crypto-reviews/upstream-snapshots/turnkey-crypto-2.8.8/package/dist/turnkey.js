...
(function (Enclave) {
    ...
    Enclave["UMP"] = "ump";
})(exports.Enclave || (exports.Enclave = {}));
/**
...
const decryptCredentialBundle = (credentialBundle, embeddedKey) => {
    try {
        const bundleBytes = encoding.bs58check.decode(credentialBundle);
...
const decryptExportBundle = async ({ exportBundle, embeddedKey, organizationId, dangerouslyOverrideSignerPublicKey, keyFormat, returnMnemonic, }) => {
    try {
        ...
        if (keyFormat === "SOLANA" && !returnMnemonic) {
            ...
            return encoding.bs58.encode(concatenatedBytes);
        }
...
const verifyEnclaveSignature = async (enclaveQuorumPublic, publicSignature, signedData, dangerouslyOverrideSignerPublicKey) => {
    const expectedSignerPublicKey = dangerouslyOverrideSignerPublicKey || constants.PRODUCTION_SIGNER_SIGN_PUBLIC_KEY;
    if (enclaveQuorumPublic != expectedSignerPublicKey) {
        throw new Error(`expected signer key ${dangerouslyOverrideSignerPublicKey ?? constants.PRODUCTION_SIGNER_SIGN_PUBLIC_KEY} does not match signer key from bundle: ${enclaveQuorumPublic}`);
    }
...
const decodeKey = (privateKey, keyFormat) => {
    switch (keyFormat) {
        case "SOLANA":
            const decodedKeyBytes = encoding.bs58.decode(privateKey);
...
const verifySessionJwtSignature = async (jwt, dangerouslyOverrideNotarizerPublicKey) => {
    const notarizerKeyHex = dangerouslyOverrideNotarizerPublicKey ??
        constants.PRODUCTION_NOTARIZER_SIGN_PUBLIC_KEY;
    /* 1. split JWT -------------------------------------------------------- */
...
const encryptToEnclave = async (targetPublicKeyUncompressed, message) => {
    return await crypto.quorumKeyEncrypt(encoding.uint8ArrayFromHexString(targetPublicKeyUncompressed), new TextEncoder().encode(message));
};
/**
...
const encryptOauth2ClientSecret = async (client_secret, dangerouslyOverrideTlsFetcherPublicKey) => {
    return encoding.uint8ArrayToHexString(await encryptToEnclave(dangerouslyOverrideTlsFetcherPublicKey ??
        constants.PRODUCTION_TLS_FETCHER_ENCRYPT_PUBLIC_KEY, client_secret));
};
/**
...
const encryptOnRampSecret = (secret, dangerouslyOverrideOnRampEncryptionPublicKey) => {
    return encoding.bs58check.encode(crypto.hpkeEncrypt({
        plainTextBuf: new TextEncoder().encode(secret),
        targetKeyBuf: crypto.uncompressRawPublicKey(encoding.uint8ArrayFromHexString(dangerouslyOverrideOnRampEncryptionPublicKey ??
            constants.PRODUCTION_ON_RAMP_CREDENTIALS_ENCRYPTION_PUBLIC_KEY)),
    }));
};

exports.decryptCredentialBundle = decryptCredentialBundle;
exports.decryptExportBundle = decryptExportBundle;
exports.encryptOauth2ClientSecret = encryptOauth2ClientSecret;
exports.encryptOnRampSecret = encryptOnRampSecret;
