...
const EnvelopeSchema = {
    struct: {
        nonce: { array: { type: "u8", len: constants.QUORUM_ENCRYPT_NONCE_LENGTH_BYTES } },
        ephemeralSenderPublic: {
            array: { type: "u8", len: constants.UNCOMPRESSED_PUB_KEY_LENGTH_BYTES },
        },
...
const quorumKeyEncrypt = async (targetPublicKeyUncompressed, message) => {
    ...
    let cipher = await createQuorumKeyEncryptCipher(encoding.uint8ArrayFromHexString(ephemeralSenderPublic), encoding.uint8ArrayFromHexString(ephemeralKeyPair.privateKey), targetPublicKeyUncompressed);
    // generate a nonce
    const nonce = new Uint8Array(constants.QUORUM_ENCRYPT_NONCE_LENGTH_BYTES);
    ...
    const aad = createAdditionalAssociatedData(encoding.uint8ArrayFromHexString(ephemeralSenderPublic), targetPublicKeyUncompressed);
    ...
    let envelope = {
        nonce: nonce,
        ephemeralSenderPublic: encoding.uint8ArrayFromHexString(ephemeralSenderPublic),
        encryptedMessage: new Uint8Array(encryptedMessageBuf),
    };
    // borsh serialize the envelope
    return borsh__namespace.serialize(EnvelopeSchema, envelope);
};
/**
...
async function createQuorumKeyEncryptCipher(ephemeralSenderPublic, ephemeralSenderPrivate, targetPublicKeyUncompressed) {
    ...
    const sharedSecretUncompressed = p256.p256.getSharedSecret(ephemeralSenderPrivate, targetPublicKeyUncompressed, false);
    ...
    const mac = new Uint8Array(await crypto.subtle.sign("HMAC", hmacKey, constants.QOS_ENCRYPTION_HMAC_MESSAGE));
...
exports.quorumKeyEncrypt = quorumKeyEncrypt;
